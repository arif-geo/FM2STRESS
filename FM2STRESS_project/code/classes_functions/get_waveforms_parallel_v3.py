...
def get_waveforms_parallel(
    ...
    priority_channels_list = ['HH*', 'BH*', 'HN*', 'EH*'],
    output_folder = None,
    ):
    """
    ...
    Args:
        ...
        priority_channels_list (list): List of priority channels to download.
    ...
    for client_name in client_list:
        ...
        for network in inventory.networks:
            for station in network.stations:
                args_list.append((client, network, station, priority_channels_list, starttime, endtime, st, inv, success_stn))
    ...
    with tqdm(total=len(args_list), disable=False) as pbar:
...
def download_station(args):
    ...
    """
    client, network, station, priority_channels_list, starttime, endtime, st, inv, success_stn = args
    st_local = Stream()
    inv_local = Inventory()

    for priority_channel in priority_channels_list:
        ch_list = list(set([ch.code[0:] for ch in station.channels])) # list(set()) removes duplicates
        ...
        # if #1 (HH*) is downloaded, skip #2 (BH*) etc. break the loop to go to the next station
...
def get_station_inventory(
    ...
    channels_string = ",".join(priority_channels_list)
