    ...
    for i in range(len(f_csv)):
        ...
        if lines[0] == "#":
            ...
            if m2:
                ...
                #print (motion_input)
                pred_res = motion_model.predict(motion_input) # predict the polarity and sharpness
                pred_fmp = (pred_res['T0D0'] + pred_res['T0D1'] + pred_res['T0D2'] + pred_res['T0D3']) / 4 # predict the polarity
                pred_cla = (pred_res['T1D0'] + pred_res['T1D1'] + pred_res['T1D2'] + pred_res['T1D3']) / 4 # predict the sharpness
