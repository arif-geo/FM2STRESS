...
%--------------------------------------------------------------------------
for i_mechanism = 1:N
% random choice between 2 options, function from the statistics toolbox    
%    choice = unidrnd(2);
% random choice between two options, function from the Matlab kernel
    choice = round(rand(1));
...
% pinv(A) gives sometimes complex-valued numbers
%--------------------------------------------------------------------------
stress_vector = real(pinv(A)*a_vector);
