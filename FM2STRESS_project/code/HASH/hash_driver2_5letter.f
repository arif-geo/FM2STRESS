...
c 		11/17/2023 Jianhua Gong
c       character*5 sname(npick0)                        ! station name length change to 5
c       35      format (a5,1x,a2,2x,a3,1x,a1,1x,a1)      ! format 35, a4->a5
c       if (sname(k).eq.'     ')  goto 40 ! end of data for this event ! not 5 empty char
c 		compile together with station_subs_5char.f, 
c       function: GETSTAT_TRI is replaces with GETSTAT_SCSN in station_subs_5char.f
c       function: check_pol is from station_subs_5char.f 
...
c file names
     ...
     &  (min ',dang0,')'
      read *,dang
      dang2=max(dang0,dang) ! donnot do finer than dang0
...
35      format (a5,1x,a2,2x,a3,1x,a1,1x,a1)
...
c stop if there aren't enough polarities
      ...
      if (npol.lt.npolmin) then
		print *,'not enough npol = ',npol,'  npolmin = ',npolmin
        str_avg(1)=999
        dip_avg(1)=99
        rak_avg(1)=999
        var_est(1,1)=99
...
c and stop if either gap is too big
      ...
      if ((magap.gt.max_agap).or.(mpgap.gt.max_pgap)) then
		print *,'magap, max_agap, mpgap, max_pgap = ',magap, max_agap, mpgap, max_pgap
        str_avg(1)=999
        dip_avg(1)=99
        rak_avg(1)=999
        var_est(1,1)=99
